...
class PlayState extends MusicBeatState
{
	...
	public static var hasfinishedcutscene:Bool = false;
	...
	public static var wrap:FlxSprite;
	...
	var dodgesprite:FlxSprite = new FlxSprite(0, 0);
	var dodgetime:Bool = false;
	...
	var background2:BGSprite;
	var background3:BGSprite;

	var vignette:BGSprite;

var myworldthing:Bool = false;
	...
	private var camMovement:FlxTween;
    var camFocus:String = "";
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'captivated': //The fucking  that is captivated bg(cool)
				var bg:BGSprite = new BGSprite('captivatedBG', 0, 0, 0.9, 0.9);
			...
			case 'larrybg': //larry bg
				var bg:BGSprite = new BGSprite('Part1', -200, -100, 0.9, 0.9);
				...
				larryeffect = new FlxSprite().loadGraphic(Paths.image('CorrosionEffect'));
				larryeffect.antialiasing = true;
				larryeffect.screenCenter();
				larryeffect.cameras = [camHUD];
				larryeffect.setGraphicSize(Std.int(larryeffect.width / 1.05));
						
				larryeffect.alpha = 0;

				fakebg = new FlxSprite().loadGraphic(Paths.image('Part2'));
					fakebg.antialiasing = true;
					fakebg.screenCenter();
					fakebg.setGraphicSize(Std.int(fakebg.width * 1.15));
					add(fakebg);
					fakebg.alpha = 0;

			case 'gocbg': //sammy bg
				var bg:BGSprite = new BGSprite('GOCBG', 0, 0, 0.9, 0.9);
			...
			case 'doctorbg': //049 bg
				var bg:BGSprite = new BGSprite('049BG', 0, 0, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1.3));
				bg.antialiasing = true;
				bg.updateHitbox();
				bg.screenCenter(Y);
			...
			case 'toiletbg': //049 bg
				var bg:BGSprite = new BGSprite('Background1', -1100, -1100, 1, 1);
				bg.antialiasing = true;
				bg.updateHitbox();
				add(bg);

				var stage_me:BGSprite = new BGSprite('Stage', -1100, -1100, 0.9, 0.9);
				stage_me.setGraphicSize(Std.int(stage_me.width * 1.1));
				stage_me.antialiasing = true;
				stage_me.updateHitbox();
				add(stage_me);

				background2 = new BGSprite('Background2', -1100, -1100, 0.8, 0.8);
				background2.antialiasing = true;
				background2.updateHitbox();
				add(background2);

				background3 = new BGSprite('Background3', -1100, -1100, 0.7, 0.7);
				background3.antialiasing = true;
				background3.updateHitbox();
				add(background3);

				var foreground:BGSprite = new BGSprite('Foreground', -1400, -1100, 0.6, 0.6);
				foreground.antialiasing = true;
				foreground.updateHitbox();
				add(foreground);

				wrap = new FlxSprite(0, 0);
				wrap.frames = Paths.getSparrowAtlas('yougottaken');
				wrap.animation.addByPrefix('wrap_enter', 'enter', 24, false);
				wrap.visible = false;
				wrap.screenCenter();

				vignette = new BGSprite('toiletvignette', 0, 0, 0.9, 0.9);
		...
		if(dad.curCharacter.startsWith('toilet')) {
			gf.visible = false;
		}
	...
	function triggerCamMovement(num:Float = 0)
    {
        ...
        if (camFocus == 'bf')
        {
            ...
            {
                case 3:
                    camMovement = FlxTween.tween(camFollow, {x: bfPos[0] + 35, y: bfPos[1]}, 0.01, {ease: FlxEase.linear});
                case 2:
                    camMovement = FlxTween.tween(camFollow, {y: bfPos[1] - 35, x: bfPos[0]}, 0.01, {ease: FlxEase.linear});
                case 1:
                    camMovement = FlxTween.tween(camFollow, {y: bfPos[1] + 35, x: bfPos[0]}, 0.01, {ease: FlxEase.linear});
                case 0:
                    camMovement = FlxTween.tween(camFollow, {x: bfPos[0] - 35, y: bfPos[1]}, 0.01, {ease: FlxEase.linear});
        ...
        else
        {
            ...
            {
                case 3:
                    camMovement = FlxTween.tween(camFollow, {x: dadPos[0] + 35, y: dadPos[1]}, 0.01, {ease: FlxEase.linear});
                case 2:
                    camMovement = FlxTween.tween(camFollow, {y: dadPos[1] - 35, x: dadPos[0]}, 0.01, {ease: FlxEase.linear});
                case 1:
                    camMovement = FlxTween.tween(camFollow, {y: dadPos[1] + 35, x: dadPos[0]}, 0.01, {ease: FlxEase.linear});
                case 0:
                    camMovement = FlxTween.tween(camFollow, {x: dadPos[0] - 35, y: dadPos[1]}, 0.01, {ease: FlxEase.linear});
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			babyArrow.y -= 170;
			babyArrow.alpha = 0;
			FlxTween.tween(babyArrow, {y: babyArrow.y + 170, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
	...
	public static function switchStatee(nextState:FlxState){
	...
	override public function update(elapsed:Float)
	{
		...
		//doctor and zombie changing icons

		if (curSong.toLowerCase() == 'vivisection')
		{
			if (dad.animation.curAnim.name.endsWith('alt'))
				{
					...
					iconP2.changeIcon('049Icons1');
				}
		}
		if (curSong.toLowerCase() == 'me')
		{
			background2.y = -1000 + Math.sin((Conductor.songPosition / 1000)*(Conductor.bpm/60) * 2.0) * 7.0;
			background3.y = -400 + Math.sin((Conductor.songPosition / 1000)*(Conductor.bpm/60) * 4.0) * 7.0;
		}
		...
		if (FlxG.keys.justPressed.SEVEN && !endingSong && !inCutscene)
		{
			...
			cancelFadeTween();
			CustomFadeTransition.nextCamera = camOther;
			switchStatee(new ChartingState());
		...
		if (FlxG.keys.justPressed.EIGHT && !endingSong && !inCutscene) {
			...
			cancelFadeTween();
			CustomFadeTransition.nextCamera = camOther;
			switchStatee(new CharacterEditorState(SONG.player2));
		}
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					if (SONG.song != 'Tutorial')
						camZooming = true;
					if(daNote.noteType == 'bullet decoration')
					{
						if (camFocus == 'dad')
							{
					...
					else if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'meeffect':
				wrap.visible = true;
				wrap.animation.play('wrap_enter', false);
			case 'myworldeffect':
				myworldthing = true;
				FlxG.camera.flash(FlxColor.WHITE, 2);
				FlxTween.tween(FlxG.camera, {zoom: 0.7}, 4, {ease: FlxEase.quadInOut, type: PINGPONG});
				fakebg.alpha = 1;
				larryeffect.alpha = 0.3;
				FlxTween.tween(larryeffect, {alpha: 0.90}, 1, {ease: FlxEase.quadInOut, type: PINGPONG});
				var blackthingy:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
				add(blackthingy);
				blackthingy.alpha = 0.5;
				blackthingy.cameras = [camHUD];
				gf.visible = false;
				if(!ClientPrefs.pussy)
					drainmult = 0.0075;
			case 'dodgemechanic':
				if(!ClientPrefs.pussy){
				dodgetime = true;
				
				dodgesprite.frames = Paths.getSparrowAtlas('dodge');
				dodgesprite.animation.addByPrefix('dodge', 'atkatkakkcktka', 24, false);
				dodgesprite.cameras = [camHUD];
				add(dodgesprite);
				dodgebeat = curBeat + 3;
				}
			case 'gunlmao':
            var gunlmao:Float = Std.parseFloat(value1);
			...
			case 'healthset':
			var healthset:Float = Std.parseFloat(value1);
			if(!ClientPrefs.pussy)
				health == healthset;
	...
	function moveCameraSection(?id:Int = 0):Void {
        if (SONG.notes[id] != null && !SONG.notes[id].mustHitSection && camFocus != "dad")
        {
            moveCamera(true);
            callOnLuas('onMoveCamera', ['dad']);
        }

        if (SONG.notes[id] != null && SONG.notes[id].mustHitSection && camFocus != "bf")
        {
            moveCamera(false);
	...
	public function moveCamera(isDad:Bool) {
        if(isDad) {
            ...
            switch (dad.curCharacter)
			{
	            ...
	            dadPos[1] = dad.getMidpoint().y - 250;

	            default:
	            dadPos[0] = dad.getMidpoint().x + 200;
            ...
            camMovement = FlxTween.tween(camFollow, {x: dadPos[0], y: dadPos[1]}, 0.05, {ease: FlxEase.quintOut});
        } else {
            camMovement.cancel();
            camFocus = 'bf';
			...
			{
				case 'larrybg':
				bfPos[0] = boyfriend.getMidpoint().x - 80;
	            bfPos[1] = boyfriend.getMidpoint().y - 250;

	            case 'gocbg':
	            bfPos[0] = boyfriend.getMidpoint().x - 120;
	            bfPos[1] = boyfriend.getMidpoint().y - 150;
            ...
            camMovement = FlxTween.tween(camFollow, {x: bfPos[0], y: bfPos[1]}, 0.05, {ease: FlxEase.quintOut});
        }
    }
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					cancelFadeTween();
					CustomFadeTransition.nextCamera = camOther;
					switchStatee(new MainMenuState());
	...
	public static function resetStatee() {
		PlayState.switchStatee(FlxG.state);
	}
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(drainmult == 0)
				...
				health += note.hitHealth + drainmult / 2;
	...
	override function stepHit()
	{
		...
		health -= drainmult;
			if (healthBar.percent < 20 && myworldthing)
				drainmult = 0.005;

			if (healthBar.percent > 80 && myworldthing)
				drainmult = 0.01;
		// if (curSong.toLowerCase() == 'my-world'){
		// 	switch (curStep){
		// 		case 640:
	...
	override function beatHit()
	{
		...
		if(dodgetime && dodgebeat == curBeat){
			dodgetime = false;
			dad.playAnim("shoot");
			dad.specialAnim = true;
			if(dodged)
		...
		if(dodgetime)
			dodgesprite.animation.play('dodge');
