...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Clearance Level One Mode':
						ClientPrefs.pussy = !ClientPrefs.pussy;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Clearance Level One Mode':
				daText = "deactivates mechanics, also known as pussy mode,baby mode, shitty stupid mod, really? mode, mode that you should deactivate, and finally. dummy silly stupid goofy mode ";
			case 'Middlescroll':
