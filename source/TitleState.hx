...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		DiscordClient.changePresence("Title screen lmaooo", null);
		#end
	...
	function startIntro()
	{
		...
		back = new FlxSprite(0, 0).loadGraphic(Paths.image('bg'));
		add(back);
		back.visible = false;
		back.updateHitbox();
		back.screenCenter();
		...
		back.alpha = 0.1;
		FlxTween.tween(back, {alpha: 0.55}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});
		...
		logoSpr = new FlxSprite(0, FlxG.height * 0.42).loadGraphic(Paths.image('scp_logo'));
		add(logoSpr);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxG.sound.music.fadeOut(1, 0, function(FlxTween:FlxTween)
				{
					FlxG.sound.music.stop();
				});
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					FlxTween.tween(logoBl, {y: logoBl.y - 1000}, 1, {ease: FlxEase.quadInOut});
				});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['The', 'friday night foundation', 'team'], 45);
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['scp'], -60);
				case 7:
					addMoreText('foundation', -60);
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Protect'); // credTextShit.text += '\nFunkin';
